...
function decodeDevice(payload) {
    ...
    switch (payload[0]) {
        ...
        case 11:
            ...
            deviceStatus = decodeDeviceThermostat(payload);
            break;
        case 12:
            deviceType = EQ3MAX_DEV_TYPE_WALLTHERMOSTAT;
            deviceStatus = decodeDeviceWallThermostat(payload);
            break;
        case 6:
            deviceType = EQ3MAX_DEV_TYPE_SHUTTER_CONTACT;
            deviceStatus = decodeDeviceShutterContact(payload);
            break;
        default:
...
function decodeDeviceWallThermostat(payload) {
    ...
    var deviceStatus = {
        ...
        temp: ((payload[12]) + ((payload[8] & 0x80) << 1)) / 10
    };
...
function decodeDeviceShutterContact(payload) {
    /*
      source: http://www.domoticaforum.eu/viewtopic.php?f=66&t=6654
      Start Length  Value       Description
      ==================================================================
      ...
      6       1     1A          bit 7     Battery       1=Low
                                ...
                                bit 3     DST setting   0=inactive,1=active
                                bit 2     Not used
                                bit 1,0   Status        00=closed
                                                        10=open
                                1A  = 00011010b
                                    = Battery OK, Linkstatus OK, Panel unlocked, Gateway known, DST active, Status open.
      */
    var deviceStatus = {
        rf_address: payload.slice(1, 4).toString('hex'),
        initialized: !!(payload[5] & (1 << 1)),
